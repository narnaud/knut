...
void SearchableViewInterface::find(const QString &text, int options)
{
    if (text.isEmpty()) {
...
void SearchableViewInterface::updateItemDelegate()
{
    m_view->setItemDelegate(new HighlightDelegate(m_view));
}

QModelIndexList SearchableViewInterface::searchModel(const QString &text, int options, const QModelIndex &parent) const
{
